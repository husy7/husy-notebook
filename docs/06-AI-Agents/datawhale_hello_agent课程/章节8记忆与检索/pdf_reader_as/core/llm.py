...
class HelloAgentsLLM:
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            think: 是否开启模型的思考模式（reasoning）。
                   None/False=禁用思考（nothink，最快直接回答）；True=默认思考级别。
                   未提供时读环境变量 LLM_THINK（"1"/"true" 视为 True）。
        """
        ...
        if self.max_tokens is None:
            try:
                self.max_tokens = int(os.getenv("LLM_MAX_TOKENS", "0")) or None
            except ValueError:
                self.max_tokens = None
        self.timeout = timeout or int(os.getenv("LLM_TIMEOUT", "60"))
        # think 模式：默认禁用思考（nothink）。None/False → 不思考；True → 思考。
        # 用 `_think_enabled` 而非 `think`，避免与下方 `think` 方法同名冲突
        # （否则 `think` 方法会被布尔属性覆盖，流式调用 `stream_invoke` 时报 bool not callable）。
        if think is None:
            think = os.getenv("LLM_THINK", "").strip().lower() in ("1", "true", "yes", "on")
        self._think_enabled = bool(think)
        # num_ctx：限制模型上下文窗口（KV cache），避免超大 context 导致生成慢/卡。
        # 本地模型（如 granite4.2:3b）默认声明 13 万上下文，必须限小。
        self.num_ctx = None
        try:
            self.num_ctx = int(os.getenv("LLM_NUM_CTX", "2048")) or None
        except ValueError:
            self.num_ctx = None
        self.kwargs = kwargs
    ...
    def think(self, messages: list[dict[str, str]], temperature: Optional[float] = None) -> Iterator[str]:
        ...
        try:
            # 纯净化调用：仅做模型调用并接收流式返回，不干预模型的上下文/缓存/思考模式。
            # 只透传调用方显式传入的 extra_body，绝不自动注入 think / num_ctx 等配置。
    ...
    def invoke(self, messages: list[dict[str, str]], **kwargs) -> str:
        ...
        """
        try:
            # 纯净化调用：仅做模型调用并接收返回，不干预模型的上下文/缓存/思考模式。
            # 只透传调用方显式传入的 extra_body，绝不自动注入 think / num_ctx 等配置。
            extra_body = kwargs.pop("extra_body", {}) or {}
            # 显式调用方参数（temperature/max_tokens 之外的），透传给 SDK，但不含 extra_body
            override_kwargs = {k: v for k, v in kwargs.items() if k not in ['temperature', 'max_tokens', 'extra_body']}
            request_kwargs = dict(
                model=self.model,
                messages=messages,
