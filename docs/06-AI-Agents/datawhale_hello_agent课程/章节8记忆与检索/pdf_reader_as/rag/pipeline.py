...
def _fallback_text_reader(path: str) -> str:
    ...
    约定：仅用于读取纯文本文件。对二进制格式（PDF/Office/图片/音视频/压缩包等）
    不尝试强读原始字节——否则会把二进制结构当正文入库，污染向量知识库。
    """
    # 已知二进制/特殊格式拒绝出文本，宁可返回空也不产生乱码
    if _is_binary_like_extension(path):
        print(f"[WARNING] 二进制格式 {os.path.splitext(path)[1]} 无可用转换器，跳过文本提取")
        return ""
    ...
    # 进一步检测内容是否夹杂大量无法打印的控制/二进制字节；若是则视为乱码
    control_count = sum(1 for ch in text if ord(ch) < 9 or 13 < ord(ch) < 32)
    if len(text) > 0 and control_count / len(text) > 0.02:
        print("[WARNING] 检测到疑似二进制内容，跳过文本提取")
        return ""
    return text


_TEMPORARILY_UNSUPPORTED_EXTS = {
    ".pdf", ".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx",
    ".jpg", ".jpeg", ".png", ".gif", ".bmp", ".tiff", ".tif", ".webp",
    ".mp3", ".wav", ".m4a", ".aac", ".flac", ".ogg",
    ".zip", ".tar", ".gz", ".rar", ".7z",
...
def _is_binary_like_extension(path: str) -> bool:
    """判断扩展名是否为二进制/结构化格式（无法用纯文本读取正确表达内容）。"""
    ext = (os.path.splitext(path)[1] or "").lower()
    return ext in _TEMPORARILY_UNSUPPORTED_EXTS
...
def _load_single_document(path: str, chunk_size: int, chunk_overlap: int, namespace: Optional[str], source_label: str) -> List[Dict]:
    """解析并分块单个文档（供并行调用）。返回该文件的 chunks（内部已按 content_hash 去重）。"""
    chunks: List[Dict] = []
    seen_hashes = set()
...
def load_and_chunk_texts(paths: List[str], chunk_size: int = 800, chunk_overlap: int = 100, namespace: Optional[str] = None, source_label: str = "rag") -> List[Dict]:
    ...
    多个文件用线程池并行解析（markitdown 含 native/IO，可并行提速），
    再在主线程做跨文件全局去重后汇总。
    """
    print(f"[RAG] Universal loader start: files={len(paths)} chunk_size={chunk_size} overlap={chunk_overlap} ns={namespace or 'default'}")
    all_file_chunks: List[List[Dict]] = []

    if len(paths) > 1:
        # 并行解析多个文件；解析本身抛异常时该文件降级为空
        with ThreadPoolExecutor(max_workers=min(len(paths), 4), thread_name_prefix="rag-load") as ex:
            future_map = {ex.submit(_load_single_document, p, chunk_size, chunk_overlap, namespace, source_label): p for p in paths}
            for fut, _ in future_map.items():
                try:
                    all_file_chunks.append(fut.result())
                except Exception as e:
                    print(f"[WARNING] 单文档并行解析失败: {e}")
        # 汇总并按跨文件 content_hash 去重
        return _dedupe_chunks(all_file_chunks)
    else:
        # 单文件常规路径，保持行为一致
        return _load_single_document(paths[0], chunk_size, chunk_overlap, namespace, source_label) if paths else []


def _dedupe_chunks(all_file_chunks: List[List[Dict]]) -> List[Dict]:
    """跨文件全局去重后拼装 chunks。"""
    chunks: List[Dict] = []
    seen_hashes = set()
...
def _create_default_vector_store(dimension: int = None) -> QdrantVectorStore:
    ...
    # 使用连接管理器（顶层绝对导入：项目根目录已在 sys.path 上）
...
def embed_queries_batch(texts: List[str]) -> List[List[float]]:
    """
    批量嵌入多个查询文本（一次 torch 前向，避免逐条循环多次前向）。

    Args:
        texts: 待嵌入的文本列表（去空后的有效项）

    Returns:
        List[List[float]]：与 texts 一一对应的向量；单个失败时用零向量兜底。
    """
    if not texts:
        return []
    embedder = get_text_embedder()
    ...
    try:
        ...
        # 兼容"单向量被误判为列表"或返回 numpy 数组的情况
        if isinstance(vecs, dict):
            out = [_norm(texts[0])] * len(texts)
            return out
        if len(vecs) == 1 and len(texts) > 1 and not isinstance(vecs[0], (list, tuple)):
            # numpy 二维数组向量化成一个向量（异常分支，兜底）
            vecs = [vecs for _ in texts]
        normed = []
        for v in vecs:
            try:
                normed.append(_norm(v))
            except Exception:
                normed.append([0.0] * dimension)
        return normed
    except Exception as e:
...
def search_vectors_expanded(
    ...
    # expansions（MQE / HyDE 的 LLM 调用相互独立，并行发起以缩短等待）
    expansions: List[str] = [query]

    if enable_mqe or enable_hyde:
        with ThreadPoolExecutor(max_workers=3, thread_name_prefix="rag-expand") as ex:
            futures = []
            if enable_mqe and mqe_expansions > 0:
                futures.append((ex.submit(_prompt_mqe, query, mqe_expansions), "list"))
            if enable_hyde:
                futures.append((ex.submit(_prompt_hyde, query), "single"))
    ...
    # 批量嵌入所有扩展查询（一次前向，避免逐条多次前向）
    qvs = embed_queries_batch(expansions)

    # 每个查询向量并发检索 Qdrant（网络 IO，并发有收益）
    def _search_one(qv):
        try:
            return store.search_similar(
                query_vector=qv,
    ...
    agg: Dict[str, Dict] = {}
    with ThreadPoolExecutor(max_workers=min(len(qvs), 8), thread_name_prefix="rag-search") as ex:
        hits_list = list(ex.map(_search_one, qvs))
...
def create_rag_pipeline(
    ...
    # 用 Qdrant 连接管理器复用同 collection 的连接，避免每次初始化重复创建索引（慢）
    from storage.qdrant_store import QdrantConnectionManager
