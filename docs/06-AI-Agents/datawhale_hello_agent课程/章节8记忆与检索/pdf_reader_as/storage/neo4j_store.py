...
if __name__ == "__main__":
    ...
    logger = logging.getLogger("Neo4jTest")

    # 检查依赖
    if not NEO4J_AVAILABLE:
        logger.error("neo4j 库未安装，请运行: pip install neo4j>=5.0.0")
        exit(1)

    # 可选：从环境变量读取连接参数，未设置则使用默认本地配置
    import os
    uri = os.getenv("NEO4J_URI", "bolt://localhost:7687")
    username = os.getenv("NEO4J_USER", "neo4j")
    password = os.getenv("NEO4J_PASSWORD", "neo4j")

    logger.info(f"连接信息: URI={uri}, User={username}")
    ...
    try:
        ...
        logger.info("请确保 Neo4j 服务已启动，或检查连接参数。")
        exit(1)
    ...
    # 1. 添加实体
    entity1_id = f"{TEST_PREFIX}entity_1"
    entity2_id = f"{TEST_PREFIX}entity_2"

    logger.info("添加实体1...")
    success = store.add_entity(
        entity_id=entity1_id,
        name="测试实体A",
        entity_type="TestType",
        properties={"description": "这是一个测试实体A"}
    )
    ...
    success = store.add_entity(
        ...
        name="测试实体B",
        entity_type="TestType",
        properties={"description": "这是一个测试实体B"}
    )
    logger.info(f"结果: {success}")

    # 2. 添加关系
    logger.info("添加关系: 实体1 -> 实体2 (KNOWS)...")
    ...
    # 3. 查找相关实体
    logger.info(f"查找与 {entity1_id} 相关的实体 (深度2)...")
    related = store.find_related_entities(entity1_id, max_depth=2)
    for ent in related:
        logger.info(f"  - {ent.get('name')} (距离: {ent.get('distance')}, 路径: {ent.get('relationship_path')})")

    # 4. 按名称搜索
    logger.info("按名称模式 '测试' 搜索实体...")
    results = store.search_entities_by_name("测试", entity_types=["TestType"])
    for ent in results:
        logger.info(f"  - {ent.get('name')} (ID: {ent.get('id')})")

    # 5. 获取关系详情
    logger.info(f"获取 {entity1_id} 的所有关系...")
    rels = store.get_entity_relationships(entity1_id)
    for rel in rels:
        other = rel['other_entity']
        direction = rel['direction']
        logger.info(f"  - {direction}: {other.get('name')} (通过 {rel['relationship'].get('type')})")
    ...
    # 7. 清理测试数据（可选，注释掉以保留数据供手动检查）
    # 这里删除所有以 test_ 开头的实体
    logger.info("正在清理测试数据...")
    # 删除实体1会自动删除其关系，但实体2可能还与实体1关联，所以需要先删除关系或一并删除
    # 简单起见，分别删除两个实体
    store.delete_entity(entity1_id)
    store.delete_entity(entity2_id)
    # 也可使用 clear_all() 但会清空整个库，谨慎使用
    logger.info("清理完成。")
