...
class RAGTool(Tool):
    ...
    def _advanced_retry(self, pipeline, query: str, limit: int, min_score: float, namespace: Optional[str]) -> List[Dict]:
        """高级检索重试：对基础检索不利的查询，用 MQE+HyDE 再检索一次。

        仅当基础检索结果不足或质量较差时调用，避免普通问题无谓消耗 LLM。
        """
        ...
        try:
            ...
            print(f"[WARNING] 高级检索失败: {e}")
            return []

    def _retrieve_on_demand(self, pipeline, query: str, limit: int, min_score: float, enable_advanced: bool, namespace: Optional[str]) -> List[Dict]:
        """按需检索：先基础检索，结果不足/整体低分时才启用 MQE+HyDE 重试。

        使 enable_advanced_search 变成"按需"而非"每题必开"，
        普通问题只走秒级的基础检索，疑难问题才触发较慢的高级检索。
        """
        score_threshold = min_score if min_score and min_score > 0 else None
        # 1) 基础检索
        ...
        if not enable_advanced:
            return results
        # 3) 判断是否需要重试：无结果，或整体质量较差
        if not results:
            return self._advanced_retry(pipeline, query, limit, min_score, namespace)
        avg_score = sum(float(r.get("score", 0.0)) for r in results) / len(results)
        if avg_score < 0.3:
            retry = self._advanced_retry(pipeline, query, limit, min_score, namespace)
            if retry:
                return retry
        return results
    ...
    def _search(self, query: str, limit: int = 5, min_score: float = 0.1, enable_advanced_search: bool = True, max_chars: int = 1200, include_citations: bool = True, namespace: Optional[str] = None, **kwargs) -> str:
        ...
        try:
            ...
            # 使用统一 RAG 管道搜索（按需高级检索：基础先行，不足才用 MQE+HyDE）
            pipeline = self._get_pipeline(namespace)
            results = self._retrieve_on_demand(
                pipeline,
                query=query,
                limit=limit,
                min_score=min_score,
                enable_advanced=enable_advanced_search,
                namespace=namespace,
            )
    ...
    def _build_qa_inputs(self, question, query, limit, enable_advanced_search, include_citations, max_chars, namespace):
        """智能问答的前置步骤：解析问题 → 检索 → 构建上下文与提示词。

        Returns:
            (user_question, enhanced_prompt, citations, search_time, avg_score)
            检索无结果时返回 (user_question, None, [], 0, 0)。
        """
        ...
        if not results:
            return user_question, None, [], search_time, 0
        ...
        return user_question, enhanced_prompt, citations, search_time, avg_score
    ...
    def _ask(self, question: Optional[str] = None, query: Optional[str] = None, limit: int = 5, enable_advanced_search: bool = True, include_citations: bool = True, max_chars: int = 1200, namespace: Optional[str] = None, **kwargs) -> str:
        ...
        """
        try:
            user_question, enhanced_prompt, citations, search_time, avg_score = self._build_qa_inputs(
                question=question, query=query, limit=limit,
    ...
    def _ask_stream(self, question: Optional[str] = None, query: Optional[str] = None, limit: int = 5, enable_advanced_search: bool = True, include_citations: bool = True, max_chars: int = 1200, namespace: Optional[str] = None, **kwargs):
        """智能问答（流式）：逐 token 生成回答。

        generator，依次 yield：
        - ("status", 消息)：进度提示（检索中 / 生成中）
        - ("delta", 文本片段)：LLM 逐 token 增量
        - ("done", 最终完整格式化答案)
        - ("error", 错误信息)
        """
        user_question, enhanced_prompt, citations, search_time, avg_score = self._build_qa_inputs(
        ...
        try:
            ...
            yield ("error", f"❌ 智能问答流式生成失败: {str(e)}")
    ...
    def validate_parameters(self, parameters: Dict[str, Any]) -> bool:
        ...
        RAG 操作的 action 为必需字段，不同动作有各自的关键参数约束。
        """
        if "action" not in parameters:
            raise ValueError("参数中必须包含 'action' 字段")

        action = parameters["action"]
        if action == "add_document" and not parameters.get("file_path"):
            raise ValueError("执行 'add_document' 动作时必须提供 'file_path' 字段")
        if action == "add_text" and not parameters.get("text"):
            raise ValueError("执行 'add_text' 动作时必须提供 'text' 字段")
        if action in ("ask",) and not (parameters.get("question") or parameters.get("query")):
            raise ValueError("执行 'ask' 动作时必须提供 'question' 或 'query' 字段")
        if action == "search" and not (parameters.get("query") or parameters.get("question")):
            raise ValueError("执行 'search' 动作时必须提供 'query' 或 'question' 字段")
    ...
    def _execute(self, parameters: Dict[str, Any]) -> str:
        ...
        复用 execute() 的真实实现，保证 run() 模板与 execute() 一致。
        """
        action = parameters.get("action")
        kwargs = {k: v for k, v in parameters.items() if k != "action"}
        return self.execute(action, **kwargs)
