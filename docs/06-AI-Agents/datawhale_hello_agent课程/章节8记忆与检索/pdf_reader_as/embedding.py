...
def create_embedding_model(model_type: str = "local", **kwargs) -> EmbeddingModel:
    ...
    if model_type in ("local", "sentence_transformer", "huggingface"):
        # 本地模型只关心模型名称
        local_kwargs = {k: v for k, v in kwargs.items() if k == "model_name"}
        return LocalTransformerEmbedding(**local_kwargs)
    elif model_type == "dashscope":
        # DashScope 只关心模型名称 / API Key / Base URL
        ds_kwargs = {k: v for k, v in kwargs.items() if k in ("model_name", "api_key", "base_url")}
        return DashScopeEmbedding(**ds_kwargs)
    elif model_type == "tfidf":
        # TF-IDF 只做轻量兜底，忽略提供商相关参数
