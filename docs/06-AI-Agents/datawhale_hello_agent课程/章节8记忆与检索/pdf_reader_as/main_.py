...
class PDFLearningAssistant:
    ...
    def __init__(self, user_id: str = "default_user"):
        ...
        # RAG 命名空间：默认搜索 "default"（既有知识库数据所在命名空间）。
        # 如需按用户隔离，设置环境变量 RAG_NAMESPACE=pdf_{user_id} 或任意自定义值。
        self.rag_namespace = os.getenv("RAG_NAMESPACE", "default")
    ...
    def ask(self, question: str, use_advanced_search: bool = True) -> str:
        ...
        直接基于 Qdrant 知识库中已索引的内容检索回答，不依赖当前会话是否加载过文档；
        若知识库为空则返回友好提示。
        ...
        # 使用RAG检索答案（无需前置加载文档，直接查询 Qdrant 已有内容）
    ...
    def ask_stream(self, question: str, use_advanced_search: bool = True):
        """向文档提问（流式生成）

        与 ask() 语义一致，但采用流式：返回生成器，依次产出：
        ("status", 进度消息) / ("delta", 文本增量) / ("done", 完整答案) / ("error", 错误信息)。
        检索与回答都基于 Qdrant 已索引内容，不依赖先加载文档。
        """
        ...
        for event in self.rag_tool._ask_stream(
            question=question,
            limit=5,
            enable_advanced_search=use_advanced_search,
...
def create_gradio_ui():
    ...
    def auto_init_assistant() -> str:
        """启动时自动初始化助手（默认 web_user），免手动点击"""
        return init_assistant("web_user")

    def load_pdf(pdf_file):
        """加载PDF文件（生成器：先反馈"处理中"，再返回结果，避免界面卡死）"""
        ...
        # 先反馈进度，让界面立即响应（依赖 queue() 流式）
        yield "📥 正在解析文档并建立知识库，请稍候..."
    ...
    def chat(message: str, history: List[Dict]):
        """聊天功能（适配 Gradio 6.0 消息格式）

        generator：技术问题走流式生成（逐 token 追加），回顾问题同步返回。
        """
        ...
        if assistant is None:
            history[-1]["content"] = "❌ 请先初始化助手"
            yield "", history
            return

        # 回顾问题：同步返回
        if any(keyword in message for keyword in ["之前", "学过", "回顾", "历史", "记得"]):
            response = assistant.recall(message)
            history[-1]["content"] = f"🧠 **学习回顾**\n\n{response}"
            yield "", history
            return
        ...
        yield "", history
        acc = ""
        for event in assistant.ask_stream(message):
            kind, payload = event
            if kind == "status":
                history[-1]["content"] = prefix + payload
            elif kind == "delta":
                acc += payload
                history[-1]["content"] = prefix + acc
            elif kind == "done":
                history[-1]["content"] = prefix + payload
            elif kind == "error":
                history[-1]["content"] = prefix + payload
    ...
    # 创建Gradio界面 - 注意：移除theme参数（已移至launch方法）
    with gr.Blocks(title="智能文档问答助手") as demo:
        ...
        with gr.Tab("💬 智能问答"):
            ...
            # 移除bubble_full_width参数（Gradio 6.0已移除）
            chatbot = gr.Chatbot(
                label="对话历史",
                height=400
            )
        ...
        # 启动时自动初始化助手（默认 web_user），免手动点击，打开页面即可使用
        demo.load(
            auto_init_assistant,
            inputs=[],
            outputs=[init_output]
...
def main():
    ...
    # 启用队列以支持流式输出（generator 触发式渲染）
    demo.queue()
    demo.launch(
        server_name="127.0.0.1",
        server_port=7860,
        share=False,
        theme=gr.themes.Soft()   # 主题参数移到这里
        # show_error 参数已移除（Gradio 6.0 中默认开启错误显示）
    )
