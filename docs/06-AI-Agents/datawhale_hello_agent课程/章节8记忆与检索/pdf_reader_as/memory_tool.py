...
class MemoryTool(Tool):
    ...
    def validate_parameters(self, parameters: Dict[str, Any]) -> bool:
        ...
        白名单与 execute() 支持的动作保持一致：
        add, search, summary, stats, update, remove, forget, consolidate, clear_all。
        （历史遗留的 "delete" 已并入 "remove"，不再接受。）
        """
        if "action" not in parameters:
            raise ValueError("参数中必须包含 'action' 字段")

        action = parameters["action"]
        valid_actions = ["add", "search", "summary", "stats", "update",
                         "remove", "forget", "consolidate", "clear_all"]
        if action not in valid_actions:
            raise ValueError(f"不支持的动作: {action}，支持的动作为: {valid_actions}")

        # 根据不同动作校验特定参数
        if action == "add" and "content" not in parameters:
            raise ValueError("执行 'add' 动作时必须提供 'content' 字段")
        if action == "search" and "query" not in parameters:
            raise ValueError("执行 'search' 动作时必须提供 'query' 字段")
        if action == "update" and "memory_id" not in parameters:
            raise ValueError("执行 'update' 动作时必须提供 'memory_id' 字段")
        if action == "remove" and "memory_id" not in parameters:
            raise ValueError("执行 'remove' 动作时必须提供 'memory_id' 字段")
    ...
    def _execute(self, parameters: Dict[str, Any]) -> str:
        ...
        复用 execute() 的真实实现，避免与 run() 的模拟逻辑产生歧义。
        """
        action = parameters["action"]
        kwargs = {k: v for k, v in parameters.items() if k != "action"}
        return self.execute(action, **kwargs)
