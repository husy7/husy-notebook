...
class SemanticMemory(BaseMemory):
    ...
    def _init_databases(self):
        ...
        设计：Qdrant 为必需（语义向量检索依赖）；Neo4j 为增强项，
        不可用（未安装 / 未启动 / 连接失败）时降级为向量-only，不阻断初始化。
        """
        ...
        try:
            ...
            # 向量存储是语义记忆的基础，缺失时应明确报错而非静默降级
            logger.error(f"❌ 数据库初始化失败: {e}")
            logger.info("💡 请检查数据库配置和网络连接")
            raise

        # 初始化Neo4j图数据库（可选，降级为向量-only）
        try:
            ...
            if not graph_health:
                logger.warning("⚠️ Neo4j连接异常，图搜索功能将受限")
                self.graph_store = None
        except Exception as e:
            logger.warning(f"⚠️ Neo4j不可用，语义记忆降级为向量-only模式: {e}")
            self.graph_store = None

        logger.info(
            f"🏥 数据库健康状态: Qdrant={'✅' if self.vector_store else '❌'}, "
            f"Neo4j={'✅' if self.graph_store else '⚠️(降级)'}"
        )
